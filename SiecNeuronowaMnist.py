...
def linear_backward(dZ, activation_history):
    ...
    dA_prev = W.T @ dZ
    #save_array_as_csv(dA_prev,"AAA_________.csv")
    #save_array_as_csv(dW,"wwwwwwww.csv")
    #save_array_as_csv(db,"bbbbbbbb.csv")
...
def backward_propagation(actual_layers, Y,  activations_history):
    ...
    #save_array_as_csv(gradient['d'+'A'+str(number_of_layers-1)],"czySoftmaxDziala.csv")
...
def initialize_parameters(layers_dims, method=InitializationMethod.RANDOM):
    ...
    for l in range(1, L):
        if method == InitializationMethod.HE:
            ...
            #save_array_as_csv(parameters['W' + str(l)],"wagi_rafala.csv")
...
def forward_propagation(X, parameters):
    ...
    activations_history = []
    A = X
    #save_array_as_csv(X,"tweeter_rafala.csv")
    ...
    #save_array_as_csv(A,"ArrajRafala.csv")
...
layers_dims = [784, 100, 50, 10] # Do każdego debila który będzie to zmieniał. PIERWSZA I OSTATNIA LICZBA NIE MA PRAWA SIĘ ZMIENIĆ !!!!!
# Do każdego debila który będzie to zmieniał. PIERWSZA I OSTATNIA LICZBA NIE MA PRAWA SIĘ ZMIENIĆ !!!!!

parameters = neural_network(train_data, train_label, layers_dims, learning_rate=0.0002, epoka=50, percent_of_validation_data=0, which_worse_prediction_stop_learning = 6)
