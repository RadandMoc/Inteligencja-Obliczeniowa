...
def crossover2(parents):
    ...
    for city in parents[0]:
        if count == crossover_point_1:
            break
        if city not in parents[1][crossover_point_1:crossover_point_2]:
            child1.append(city)
            count += 1       
    ...
    for city in parents[1]:
        if count == crossover_point_1:
            break
        if city not in parents[0][crossover_point_1:crossover_point_2]:
            child2.append(city)
            count += 1      
...
def reverse_subarray(arr, i, j):
    reverse_array = arr.copy()
    reverse_array[i:j + 1] = reverse_array[i:j + 1][::-1]
...
def genetic_algorithm(distances, population_size=100, generations=5000, mutation_rate=0.01, number_of_crossover=2000, elite_percent=0.1, iteration_without_improvement=200, history_of_best_results_for_generation=False, allow_dynamic_mutation=False):
    ...
    for generation in tqdm(range(generations)):
        ...
        if generation == 0:
            ...
            if number_without_improvement == iteration_without_improvement:
                break
        best_score_of_pregeneration = fitness_scores[best_index_of_generation]
        if history_of_best_results_for_generation and number_without_improvement == 0:
    ...
    save_data(population[best_index], fitness_scores[best_index], population_size, generations, number_of_crossover, mutation_rate, iteration_without_improvement)
